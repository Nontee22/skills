---
name: article-learning-pipeline
description: End-to-end article learning pipeline for technical study materials across Vue2/Vue3, WeChat Mini Program frontend, AI Agent systems, Java, and similar engineering domains. Use when Codex needs to extract source-traceable P-xxx AI deep-dive prompt cards from long .txt/.md articles, classify points with a domain taxonomy, write the result with output-template.md, optionally split an already template-formatted Markdown by "### 建议学习顺序", dispatch chapter workers, preserve P-xxx/SUP-xxx/interview/low-confidence sections, and merge chapters into a learning-order final document.
---
...
1. **Extract** a raw technical article into a complete `output-template.md` style digest with source-traceable `P-xxx` cards.
...
- **Raw technical article input**: use extraction mode.
...
Use this mode for long `.txt` or `.md` articles about Vue2/Vue3, WeChat Mini Program frontend, AI Agent systems, Java, or another engineering topic.
...
4. Determine the primary domain first. Use `frontend-vue`, `frontend-miniprogram`, `agent`, `java`, or `general-engineering` taxonomy labels.
5. Extract atomic article-derived points section by section. Every `P-xxx` point must have source IDs and evidence level.
6. Keep extension separated:
...
7. Write the complete result to the manifest `result_path` or to `<article directory>/结果.md`.
...
- Treat version-sensitive framework/platform/model/runtime claims as low-confidence unless verified against official docs, release notes, or source code.
